...
export class SlotsComponent implements OnInit, OnDestroy {
  ...
  private addSlotSubscription: Subscription | null = null;
  private editSlotSubscription: Subscription | null = null;
  private deleteSlotSubscription: Subscription | null = null;
  ...
  addSlot(data: Slot[]) {
    this.addSlotSubscription = this.service.addSlot(data).subscribe({
      next: (res) => {
        if (res && res.message) {
  ...
  editSlot(data: Slot) {
    this.editSlotSubscription = this.service.editSlot(this.currentslotid, data).subscribe({
      next: (res) => {
        if (res && res.message) {
          this.messageService.add({ severity: 'success', summary: 'Success', detail: res.message });
  ...
  deleteSlot(slotId: string) {
    this.deleteSlotSubscription = this.service.deleteSlot(slotId).subscribe({
      next: (res) => {
        if (res && res.message) {
          this.fetchSlots();
        }
  ...
  ngOnDestroy() {
    ...
    if (this.addSlotSubscription) {
      this.addSlotSubscription.unsubscribe();
    }
    if (this.editSlotSubscription) {
      this.editSlotSubscription.unsubscribe();
    }
